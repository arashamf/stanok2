...
void delay_us(uint16_t delay)
{
  LL_TIM_SetAutoReload(TIM7, delay); //
	LL_TIM_ClearFlag_UPDATE(TIM7); //сброс флага обновлени€ таймера
	LL_TIM_SetCounter(TIM7, 0); //сброс счЄтного регистра
	LL_TIM_EnableCounter(TIM7); //включение таймера
	while (LL_TIM_IsActiveFlag_UPDATE(TIM7) == 0) {} //ожидание установки флага обновлени€ таймера 
...
void timer_bounce_init (void)
{
  ...
  LL_TIM_DisableARRPreload(TIM6);	
	...
	LL_TIM_ClearFlag_UPDATE(TIM6); //сброс флага обновлени€ таймера
...
void repeat_time (uint16_t delay)
{
  LL_TIM_SetAutoReload(TIM6, 2*delay); //
	LL_TIM_SetCounter(TIM6, 0); //сброс счЄтного регистра
	LL_TIM_ClearFlag_UPDATE(TIM6); //сброс флага обновлени€ таймера
